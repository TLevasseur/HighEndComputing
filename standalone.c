...
int main(){
	...
	if(getconfig(config, filename)==0){
		printf("FATAL ERROR: Config file not properly loaded\n");
		return -1;
	}
	...
	if(setoptions(config) == 0){
		printf("FATAL ERROR: Incorrect configuration file\n");
		return -1;
	}
	//Here we chain the call to metis with the options string
	char command[500], temp1[150], temp2[150];
	int index=0, check=0;
	FILE *fp;
	sprintf(command,"");
	...
	if(mode == 0){
		do{
			index++;
			sprintf(temp1,"%s.out.parts.%d.%d", filename, nparts, index);
			fp=fopen(temp1,"r");
			if(fp != NULL) fclose(fp);
			else check=1;
		}while(check==0);
		sprintf(command, "mv %s.part.%d %s\n",filename, nparts, temp1);
		system(command);
	}
	else if(mode == 1){
		do{
			index++;
			sprintf(temp1, "%s.out.nparts.%d.%d", filename, nparts, index);
			sprintf(temp2, "%s.out.eparts.%d.%d", filename, nparts, index);
			fp=fopen(temp1,"r");
			if(fp!=NULL) fclose(fp);
			else check=1;
		}while(check==0);
		sprintf(command, "mv %s.npart.%d %s\n",filename , nparts, temp1);
		system(command);
		sprintf(command, "chmod 777 %s\n", temp1);
		system(command);
		sprintf(command, "mv %s.epart.%d %s\n",filename , nparts, temp2);
		system(command);
		sprintf(command, "chmod 777 %s\n", temp2);
	}
...
*  an integer array
*  @param config Pointer to array of integers with the configuration parameters
*  @param filename Pointer tthe string where we will store the filename of the 
*		file to partition
